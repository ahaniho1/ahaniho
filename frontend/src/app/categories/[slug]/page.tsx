...
export default function CategoryPage() {
  ...
  const name = slug ? slug.replace(/-/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase()) : '';

  useEffect(() => { if (slug) fetchArticles(); }, [slug]);
  ...
  const fetchArticles = async () => {
    try {
      ...
      setArticles(list.filter((a: any) => a.status === 'published' && a.categoryId?.slug === slug));
    ...
    <div style={{ background: '#F8FAFC', minHeight: '100vh' }}>
      <section style={{ background: 'linear-gradient(135deg, #EFF6FF, #F8FAFC)', padding: '48px 0', textAlign: 'center' }}>
        <div className="container">
          <h1 className="text-3xl md:text-4xl font-extrabold text-gray-900 mb-2">{name}</h1>
          <p className="text-gray-500">Explore articles about {name.toLowerCase()}</p>
        </div>
      </section>
        ...
        <div className="container">
          {loading ? (
            ...
            <div className="empty-state">
              ...
              <p className="empty-state-title">No articles in this category yet</p><p className="empty-state-desc">Check back soon.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 stagger">
              {articles.map((a: any) => (
                <Link key={a._id} href={`/articles/${a.slug}`} className="article-row" style={{ display: 'flex', flexDirection: 'column', gap: '10px', alignItems: 'stretch', padding: '18px' }}>
                  <div>
                    <span className={`badge ${getBadge(slug || '')}`} style={{fontSize:'10px',padding:'2px 8px'}}><span className="badge-dot"></span>{a.categoryId?.name || 'General'}</span>
                    <h3 className="text-xl font-bold mt-2 mb-0 leading-snug">{a.title}</h3>
                  </div>
                  <div className="article-row-thumb" style={{ width: '100%', maxWidth: '360px', height: '230px' }}>
