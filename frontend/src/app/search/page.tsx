  ...
  const handleSearchDirect = async (q: string) => {
    ...
    try {
      ...
      setResults(list.filter((a: any) => {
        ...
        const searchable = [a.title||'', a.excerpt||'', a.categoryId?.name||'', ...(a.tags||[])].join(' ').toLowerCase();
        return queryLower.split(/\s+/).every((word: string) => searchable.includes(word));
      }));
      ...
      <section style={{ background: 'linear-gradient(135deg, #EFF6FF, #F8FAFC)', padding: '40px 0 32px', textAlign: 'center' }}>
        <div className="container" style={{ maxWidth: '640px' }}>
          ...
          <form onSubmit={handleSearch} className="search-box" style={{ margin: '0 auto' }}>
            <input type="text" placeholder="Search by title, keyword, or topic..." value={query} onChange={e => setQuery(e.target.value)} />
            ...
            <div className="space-y-4 stagger">
              {results.map((a: any) => (
                <Link key={a._id} href={`/articles/${a.slug}`} className="card" style={{ display: 'flex', flexDirection: 'column', gap: '10px', padding: '16px' }}>
                  <div>
                    <span className={`badge ${getBadge(a.categoryId?.slug || '')}`} style={{ marginBottom: '4px' }}><span className="badge-dot"></span>{a.categoryId?.name || 'General'}</span>
                  ...
                  <div style={{ width: '100%', height: '180px', borderRadius: '10px', overflow: 'hidden', background: '#F1F5F9' }}>
                    {a.featuredImageUrl ? <img src={a.featuredImageUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> :
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#94A3B8" strokeWidth="1"><rect x="3" y="3" width="18" height="18" rx="3"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/></svg>
                      </div>}
                  </div>
                  <div>
                    <p className="text-gray-500 text-sm">{a.excerpt || 'Read more...'}</p>
                  </div>
                </Link>
              ))}
            </div>
